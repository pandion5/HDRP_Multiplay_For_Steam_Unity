...
//		}
		...
		if(!bWebColorStream)
		{
			...
			if (colorSensor != null) 
			{
				...
				while ((colorFrame == null || colorFrame.Cols == 0 || colorFrame.Rows == 0) && 
...
//			sensorData.colorImageHeight = colorSensor != null ? colorSensor.GetOutputMode().YRes : 480;
		...
		if (depthSensor != null) 
		{
			...
			while ((depthFrame == null || depthFrame.Cols == 0 || depthFrame.Rows == 0) && 
				Time.realtimeSinceStartup <= waitTillTime) 
			{
				nuitrack.Nuitrack.Update();
				System.Threading.Thread.Sleep(50);
	...
	public bool PollColorFrame (KinectInterop.SensorData sensorData)
	{
		...
		else if(colorFrame != null && (long)colorFrame.Timestamp != lastColorFrameTimestamp) 
		{
			...
			if (dontHFlipFrame) 
			{
                ...
                {
                    int iRow = i / 3;
                    int iNext = iRow + sensorData.colorImageWidth;

                    for (int s = iRow, d = i; s < iNext; s++, d += 3)
                    {
                        nuitrack.Color3 clr = colorFrame[s];

                        sensorData.colorImage[d] = clr.Red;
                        sensorData.colorImage[d + 1] = clr.Green;
                        sensorData.colorImage[d + 2] = clr.Blue;
                    }
                }
            }
            else 
			{
                ...
                for (int i = 0; i < lenColorFrame; i += lenColorRow)
                {
                    ...
                    for (int s = iRow, d = iNext - 3; s < iNext; s++, d -= 3)
                    {
                        nuitrack.Color3 clr = colorFrame[s];
	...
	public bool PollDepthFrame (KinectInterop.SensorData sensorData)
	{
		...
		if(depthFrame != null && (long)depthFrame.Timestamp != lastDepthFrameTimestamp) 
		{
			...
			if (dontHFlipFrame) 
			{
                ...
                var pDstData = GCHandle.Alloc(sensorData.depthImage, GCHandleType.Pinned);
                Memcpy(pDstData.AddrOfPinnedObject(), depthFrame.Data, copyLength);
                pDstData.Free();
            }
            else 
			{
